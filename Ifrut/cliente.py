...
class pessoa:
    ...
    def __init__(self,nome,cpf,endereco,bairro,cidade,estado,tel,data,usuario,senha):
        c = Conexao()
        c.connect()  
        self.nome = nome
        self.cpf = cpf
    ...
    def inserir(self):
        try:
            ...
            clientenovo =(self.nome,self.cpf,self.endereco,self.bairro,self.cidade,self.estado,self.tel,self.data,self.usuario,self.senha)
            c.execute("INSERT INTO clientes(nome,cpf,endereco,bairro,cidade,estado,tel,data,usuario,senha) VALUES (?,?,?,?,?,?,?,?,?,?);", clientenovo)
